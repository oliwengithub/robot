    <%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8"%>
        ...
        <input type="text" name="name" autocomplete="off" class="layui-input" lay-verify="required" maxlength="30" value="${menu.name}">
        </div>
        </div>
        ...
        <div class="layui-input-block">
        <input type="url" name="url" autocomplete="off" class="layui-input" maxlength="100" value="${menu.url}"  placeholder="多个菜单用英文逗号分开，如：url1,url2">
        </div>
        </div>
        <div class="layui-form-item">
        ...
        <div class="layui-input-block">
        <input type="radio" name="type" value="0" title="菜单" lay-filter="type" <c:if test="${menu.type == 0}">checked</c:if>>
        <input type="radio" name="type" value="1" title="按钮" lay-filter="type" <c:if test="${menu.type == 1}">checked</c:if>>
        <input type="radio" name="type" value="2" title="操作" lay-filter="type" <c:if test="${menu.type == 2}">checked</c:if>>
        </div>
        </div>
        <div class="layui-form-item <c:if test="${menu.type != 1}">hidden</c:if>" id="tag">
        <label class="layui-form-label">权限标识</label>
        <div class="layui-input-inline">
        <input type="text" name="tag" placeholder="控制操作按钮的显/隐标签" autocomplete="off" class="layui-input" maxlength="50" value="${menu.tag}">
        </div>
        </div>
        <div class="layui-form-item">
        <label class="layui-form-label">排序</label>
        <div class="layui-input-inline">
        <input type="number" name="sort" autocomplete="off" class="layui-input" value="${menu.sort}">
        </div>
