            ...
            <div class="layui-card">
                <div class="layui-card-body" pad15>
                    <form class="layui-form" action="">
                        <input type="hidden" name="id" value="${tradeThread.id}">
                                ...
                                <select name="tradePlatformId" lay-verify="required" lay-search="">
                                    ...
                                    <c:if test="${not empty tradePlatforms}">
                                        <c:forEach items="${tradePlatforms}" var="tradePlatform">
                                            <option value="${tradePlatform.id}" <c:if test="${tradePlatform.id == tradeThread.tradePlatformId}">selected</c:if> >${tradePlatform.tradeName}</option>
                                        </c:forEach>
                                    </c:if>
                                </select>
                            </div>
                                ...
                                <select name="tradeConfigId" lay-verify="required" lay-search="">
                                    ...
                                    <c:if test="${not empty tradeConfigs}">
                                        <c:forEach items="${tradeConfigs}" var="tradeConfig">
                                            <option value="${tradeConfig.id}" <c:if test="${tradeConfig.id == tradeThread.tradeConfigId}">selected</c:if> >${tradeConfig.symbol}</option>
                                        </c:forEach>
                                    </c:if>
                                </select>
                            </div>
                        ...
                        <div class="layui-form-item">
                            ...
                            <div class="layui-input-block">
                                <input type="number" name="tradeMin" value="${tradeThread.tradeMin}" placeholder="默认单笔挂单最小限额(CNY)" autocomplete="off" class="layui-input">
                            </div>
                        </div>
                        <div class="layui-form-item">
                            <label class="layui-form-label">最大限额</label>
                            <div class="layui-input-block">
                                <input type="number" name="tradeMax" value="${tradeThread.tradeMax}" placeholder="默认单笔挂单最大限额(CNY)" autocomplete="off" class="layui-input">
                            </div>
                        </div>
                        <div class="layui-form-item">
                            <label class="layui-form-label">波动比例</label>
                            <div class="layui-input-block">
                                <input type="number" name="waveRatio" value="${tradeThread.waveRatio}" placeholder="单价波动比例(在原始价格上波动一般为万分之一左右)" autocomplete="off" class="layui-input">
                            </div>
                        </div>
                        <div class="layui-form-item">
